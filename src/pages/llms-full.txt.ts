...
export const GET: APIRoute = async ({ site }) => {
  ...
  const header = [
    "# ai-notes — full corpus",
    "",
    "> Open library of real-world human-AI collaboration case files. Every case inlined in Markdown for direct agent consumption.",
    "",
    `Generated at: ${new Date().toISOString()}. Canonical source: ${base}/cases.json`,
    "",
    "---",
    "",
  ];
  ...
  const sections = entries.map((entry) => {
    ...
    return [
      ...
      `- **anonymized**: ${meta.anonymized ? "yes" : "no"}`,
