...
| Path | Purpose |
|---|---|
| `/robots.txt` | AI crawlers (GPTBot, ClaudeBot, PerplexityBot, CCBot, Google-Extended, Applebot-Extended) allow-listed |
...
| `/rss.xml` | RSS feed |
| JSON-LD `Article` | Embedded in every case page `<head>` |
...
- **Never re-add `@astrojs/sitemap` at its 3.7.x version** — it crashes on `astro:build:done` under Astro 4 (expects an Astro 5 hook signature). Revisit when the project upgrades to Astro 5.
...
- Sitemap (waiting for Astro 5 upgrade)
- OpenGraph image generation per case
